...
def to_pil_image(gl_img):
    ...
    if gl_img._format_enum == _format['RAW']:
        ...
        if gl_img.channels == 1:
            img = Image.frombytes('L', (gl_img._width, gl_img._height), str(gl_img._image_data))
        elif gl_img.channels == 3:
            img = Image.frombytes('RGB', (gl_img._width, gl_img._height), str(gl_img._image_data))
        elif gl_img.channels == 4:
            img = Image.frombytes('RGBA', (gl_img._width, gl_img._height), str(gl_img._image_data))
        else:
            raise ValueError('Unsupported channel size: ' + str(gl_img.channels))
    else:
        img = Image.open(_StringIO.StringIO(gl_img._image_data))
    return img
